...
4 and 7 day exposures of cytokine stimulated HIV- human PBMC from 2 donrs to various nanoparticle formulations followed by phenotyping and viability measurements by flow cytometry.
...
#day_both <- day_both[,c(-1,-11)]
#Removing columes 1,11 and 12 because they were empty/useless.


#day7<- inner_join(day7, plate_mappings)
...
Effect of free Nanoparticles (Not conjugated with MPA) on different targetings.
...
ggplot(targeting_only, aes(x = NP_mg_per_mL, y = Live, color = Donor)) + 
  scale_x_log10()+
    ...
    xlab("Nanoparticle Concentration")+
  stat_summary(fun.y = "mean", geom = "line", aes(group = Donor))
...
Comparing plate 1 and 5 with 2 and 6 - Comparing cells with and without MAP on day 4 and 7.
```{r }
...
Day 4: Percentage in gate in high count data where x axis = MPA Concentration.
```{r }
...
Day 7: Percentage in gate in high count data where x axis = MPA Concentration..
...
Day 4: Percentage in gate in high count data where x axis = NP Concentration.
```{r }
...
ggplot(., aes(x = NP_mg_per_mL, Measurement )) +
  ...
  stat_summary(fun.y = "mean", geom = "line", aes(group = Gate, color = Gate)) +
  facet_grid(Donor~Targeting)+
  labs(x = "NP-mg/mL(log10)", y = "Percent in gate")+
  theme(strip.text = element_text(size = 8)) + 
  scale_x_log10()

```

Day 7: Percentage in gate in high count data  where x axis = NP Concentration.
